...
Contiene las funciones necesarias para construir y resolver el sistema A x = b
...
def _build_linear_system(board: List[List[int]]) -> Tuple[List[List[int]], List[int]]:
    ...
    for i in range(n):
        for j in range(n):
            ...
            A[idx][idx] = 1
            if i > 0:
                A[idx][_index(i - 1, j, n)] = 1
            if i < n - 1:
                A[idx][_index(i + 1, j, n)] = 1
            if j > 0:
                A[idx][_index(i, j - 1, n)] = 1
            if j < n - 1:
                A[idx][_index(i, j + 1, n)] = 1
            b[idx] = 1 if board[i][j] else 0
...
def gauss_elimination_gf2(A: List[List[int]], b: List[int]) -> Optional[List[int]]:
    ...
    # Forward elimination: crear pivote con Fi = Fi + Fj si es necesario
    for i in range(N):
        if A[i][i] == 0:
            for j in range(i + 1, N):
                if A[j][i] == 1:
                    # allowed op: Fi = Fi + Fj (XOR)
                    _row_xor(A[i], A[j])
                    b[i] ^= b[j]
                    break
    ...
    # Back substitution: variables libres (A[i][i]==0) se fijan a 0
    x = [0] * N
...
def solve_lights_out(board: List[List[int]]) -> List[List[int]]:
    ...
    A, b = _build_linear_system(board)
    ...
    # return solution as n x n matrix
